...
static inline int iclamp(int n, int l, int h)
{
    return imax(l, imin(n, h));
}

static inline float fclamp(float n, float l, float h)
{
    return fmax(l, fmin(n, h));
}
...
void savepng(const char *filename, uchar *data, int w, int h, int bpp, int flip)
{
    ...
    switch(bpp)
    {
        ...
        default:
        {
            fatal("tessfont: invalid PNG bpp");
            return;
        }
    }
    f = fopen(filename, "wb");
    if(!f)
    {
        fatal("tessfont: could not write to %s", filename);
        return;
    }
...
{
    return cubectype[c]&CT_PRINT;
}

int iscubespace(uchar c)
{
    return cubectype[c]&CT_SPACE;
}

int iscubealpha(uchar c)
{
    return cubectype[c]&CT_ALPHA;
}

int iscubealnum(uchar c)
{
    return cubectype[c]&(CT_ALPHA|CT_DIGIT);
}

int iscubelower(uchar c)
{
    return cubectype[c]&CT_LOWER;
}

int iscubeupper(uchar c)
{
    return cubectype[c]&CT_UPPER;
}
...
struct fontchar
{
    int code, uni, tex, x, y,
        sdfradius, sdfpitch,
        sdfx, sdfy, sdfw, sdfh;
    float w, h, left, top, advance;
...
{
    int w = c->glyph->bitmap.width,
        h = c->glyph->bitmap.rows,
        radius = c->sdfradius*supersample;
    int dx, dy,
        dw = (w + 2*radius + supersample-1)/supersample,
        dh = (h + 2*radius + supersample-1)/supersample;
    int x, y,
        x1 = INT_MAX,
        y1 = INT_MAX,
        x2 = INT_MIN,
    ...
    if(!dst)
    {
        fatal("tessfont: failed allocating signed distance field");
    }
    c->sdfpitch = dw;
    c->sdf = dst;
    for(dy = 0; dy < dh; dy++)
...
void writetexs(const char *name, struct fontchar *chars, int numchars, int numtexs, int tw, int th)
{
    ...
    if(!pixels)
    {
        fatal("tessfont: failed allocating textures");
    }
...
void writecfg(const char *name, struct fontchar *chars, int numchars, float x1, float y1, float x2, float y2, int sw, int sh, int argc, char **argv)
{
    ...
    if(!f)
    {
        fatal("tessfont: failed writing %s", file);
    }
    printf("tessfont: writing %d chars to %s\n", numchars, file);
    fprintf(f, "//");
    for(i = 1; i < argc; i++)
    ...
    if(scale > 0)
    {
        fprintf(f, "fontscale %d\n", scale);
    }
    if(border2)
    {
        fprintf(f, "fontborder %g %g\n", border, border2);
    }
    else if(border)
    {
        fprintf(f, "fontborder %g\n", border);
    }
    if(outline2)
    {
        fprintf(f, "fontoutline %g %g\n", outline, outline2);
    }
    else if(outline)
    {
        fprintf(f, "fontoutline %g\n", outline);
    }
    for(i = 0; i < numchars; i++)
    {
        struct fontchar *c = &chars[i];
        ...
        float offx = c->sdfx-c->sdfradius + c->left + offsetx,
              offy = c->sdfy-c->sdfradius + y2-c->top + offsety;
        if(c->code != c->uni)
        {
...
{
    const struct fontchar *xc = *(const struct fontchar **)x,
                          *yc = *(const struct fontchar **)y;
    ...
    if(xc->sdfh != yc->sdfh)
    {
        return yc->sdfh - xc->sdfh;
    }
    if(xc->sdfw != yc->sdfw)
    {
        return yc->sdfw - xc->sdfw;
    }
...
int main(int argc, char **argv)
{
    ...
    for(c = 0; c < 256; c++)
    {
        if(iscubeprint(c))
        {
            ...
            if(!numgen)
            {
                printf("tessfont: generating %d", dst->code);
            }
            else
            {
                printf(" %d", dst->code);
